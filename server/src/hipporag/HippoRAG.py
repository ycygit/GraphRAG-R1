...
class HippoRAG:
    ...
    def index(self, docs: List[str]):
        ...
        # ner_results_dict = ner_results_dict[:10]
        # triple_results_dict = triple_results_dict[:10]
        # print(len(chunk_to_rows))
        # print(len(ner_results_dict))
        # print(len(triple_results_dict))
        ...
        entity_nodes = [item for item in entity_nodes if item]
    ...
    def retrieve(self,
        ...
        for q_idx, query in tqdm(enumerate(queries), desc="Retrieving", total=len(queries)):
            ...
            print(top_k_fact_indices, top_k_facts)
            ...
            if len(rerank_log["facts_after_rerank"])>0:
                retrieval_results.append(QuerySolution(question=query, docs=top_k_docs, doc_scores=sorted_doc_scores[:num_to_retrieve],facts=rerank_log["facts_after_rerank"]))
            else:
                retrieval_results.append(QuerySolution(question=query, docs=top_k_docs, doc_scores=sorted_doc_scores[:num_to_retrieve],facts=rerank_log["facts_before_rerank"]))
    ...
    def rag_qa(self,
               ...
               gold_answers: List[List[str]] = None,only_retrive=False,retrieval_num=3) -> Tuple[List[QuerySolution], List[str], List[Dict]] | Tuple[List[QuerySolution], List[str], List[Dict], Dict, Dict]:
        """
        ...
        if not isinstance(queries[0], QuerySolution):
            if gold_docs is not None:
                ...
                queries = self.retrieve(queries=queries,num_to_retrieve=retrieval_num)
    ...
    def get_top_k_weights(self,
        ...
        #debug
        print("linking_score_map keys:", linking_score_map.keys())
        print("top_k_phrases_keys:", top_k_phrases_keys)
        nonzero_indices = np.nonzero(all_phrase_weights)[0]
        print("nonzero_indices:", nonzero_indices)
        print("nonzero count:", np.count_nonzero(all_phrase_weights))
        print("node_name_to_vertex_idx keys:", list(self.node_name_to_vertex_idx.keys())[:10])  # 只打印前10个
        #debug
