...
client = OpenAI(
    ...
    base_url=os.getenv("RERANK_BASE_URL", "https://dashscope.aliyuncs.com/compatible-mode/v1"),
)

# 过滤后的事实列表{主语，谓语，宾语}，由模型生成
class Fact(BaseModel):
    fact: list[list[str]] = Field(description="A list of facts, each fact is a list of 3 strings: [subject, predicate, object]")


# 过滤掉不相关的，只保留相关的，按照相似度重新排序
class DSPyFilter:
    ...
    def call_ollama(self, messages, model, **kwargs):
        ...
        if "max_completion_tokens" in kwargs:
            payload["num_predict"] = kwargs["max_completion_tokens"]
    ...
    def llm_call(self, question, fact_before_filter):
        ...
        # 从环境变量读取模型名称，默认使用 qwen-turbo-latest
        ...
        try:
            ...
            print(f"LLM call failed for model {rerank_model}: {e}")
            response = ""
